...
- 实现 **Baseline 与 Draft-verifier** 对比，并在文档中明确：属于**应用层编排**，**非 token 级推测解码**。  
- 使用 **Prisma + PostgreSQL** 持久化运行与工具 trace，提供 `/runs/[id]` 时间线与**产品解读**文案。  
- 搭建**可插拔评估器**（默认启发式 + 可选 LLM 评审），在演示稳定性与关键时刻的深度打分之间取得平衡。
